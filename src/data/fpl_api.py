...
class FPLAPICollector:
    ...
    def _get_cache_path(self, endpoint: str, params: str = "") -> Path:
        ...
        filename = f"{endpoint}_{params}_{timestamp}.json" if params else f"{endpoint}_{timestamp}.json"
    ...
    async def get_fixtures(self, gameweek: Optional[int] = None, use_cache: bool = True) -> List[Dict]:
        """Get fixtures data
