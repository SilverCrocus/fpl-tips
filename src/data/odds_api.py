...
class PlayerPropsCollector:
    ...
    def __init__(self, api_key: Optional[str] = None, cache_dir: str = "cache/player_props"):
        ...
        if not self.api_key:
            raise ValueError("The Odds API key is required. Please set ODDS_API_KEY in your .env file")
    ...
    async def get_player_goal_scorer_odds(self, event_id: str, 
                                         event_name: str = "") -> Dict:
                        ...
                        if market['key'] == 'player_goal_scorer_anytime':
                            ...
                            for outcome in outcomes:
                                ...
                                if not player_name:
                                    ...
                                    player_name = outcome.get('name', '').replace(' - Anytime Goalscorer', '')
                                ...
                                if player_name and odds > 0:
                                    all_player_odds.append({
                                        ...
                                        'prob_goal': round(1 / max(odds, 0.01), 3) if odds > 0 else 0,
    ...
    async def get_player_assists_odds(self, event_id: str, 
                                     event_name: str = "") -> Dict:
    ...
    def match_players_to_fpl(self, player_odds_df: pd.DataFrame, 
                            fpl_players_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'player_name' not in player_odds_df.columns and 'bookmaker_name' in player_odds_df.columns:
        ...
        player_summary = player_odds_df.groupby('player_name').agg({
        ...
        for _, match in matches[matches['matched']].iterrows():
            ...
            odds_row = player_summary[
                player_summary['bookmaker_name'] == match['bookmaker_name']
            ]
        ...
        if matched_odds:
            ...
            logger.info(f"Matched {matched}/{total_fpl} FPL players to bookmaker odds ({matched*100/total_fpl:.1f}%)")
            ...
            if not player_odds_df.empty:
                ...
                for match_id, match_group in player_odds_df.groupby('match_id'):
                    ...
                    for _, player in top_scorers.iterrows():
                        print(f"  {player['player_name']:20} {player['odds_goal_anytime']:6.2f} "
