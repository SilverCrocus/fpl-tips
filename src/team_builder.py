...
class TeamBuilder:
    ...
    def _display_current_team(self):
        ...
        console.print(
            f"Budget: £{self.team.remaining_budget:.1f}m remaining (£{self.team.total_cost:.1f}m spent)"
        )
        console.print("=" * 80)
    ...
    def _add_player(self, position: str):
        ...
        console.clear()
        console.print(
            Panel.fit(f"[bold]Add {self._get_position_name(position)}[/bold]", border_style="cyan")
        )
        ...
        position_players = position_players[
            ~position_players["player_id"].isin(self.team.player_ids)
        ]
        ...
        if filter_choice == 2:
            ...
            team_choice = IntPrompt.ask(
                "Select team", choices=[str(i) for i in range(1, len(teams) + 1)]
            )
            ...
            max_price_input = float(Prompt.ask("Maximum price (£m)", default=str(max_price)))
            position_players = position_players[
                (position_players["price"] >= min_price)
                & (position_players["price"] <= max_price_input)
            ]
        ...
        while True:
            ...
            # Create player table
            table = Table(
                title=f"Select {self._get_position_name(position)} (Page {page + 1}/{total_pages})",
                box=box.ROUNDED,
            )
            ...
            if choice == "B":
                ...
                if 0 <= player_idx < len(page_players):
                    ...
                    console.print(
                        f"[green]✓ Added {selected_player['player_name']} to team![/green]"
                    )
                    Prompt.ask("Press Enter to continue")
                    return
    ...
    def _remove_player(self):
        ...
        choice = IntPrompt.ask(
            "Select player to remove (0 to cancel)",
    ...
    def _show_team_stats(self):
        ...
        console.print(
            f"  Average player cost: £{self.team.total_cost / max(1, self.team.team_size):.1f}m"
        )
        ...
        sorted_players = sorted(
            self.team.all_players, key=lambda x: x.get("total_points", 0), reverse=True
        )[:5]
    ...
    def _finalize_team(self) -> TeamSelection:
        ...
        captain_choice = IntPrompt.ask(
            "Select captain", choices=[str(i) for i in range(1, len(all_players) + 1)]
        )
        ...
        vc_choice = IntPrompt.ask(
            "Select vice-captain (different from captain)",
            choices=[str(i) for i in range(1, len(all_players) + 1) if i != captain_choice],
        )
    ...
    def _get_position_name(self, position: str) -> str:
        ...
        return {"GK": "Goalkeeper", "DEF": "Defender", "MID": "Midfielder", "FWD": "Forward"}.get(
            position, position
        )
