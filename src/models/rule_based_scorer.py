...
class ScoringWeights:
    ...
    # Removed default() method - weights must be explicitly provided
...
class RuleBasedScorer:
    ...
    def __init__(self, weights: ScoringWeights):
        ...
        Args:
            weights: Scoring weights (required)
        """
        if weights is None:
            raise ValueError("Scoring weights must be provided")
    ...
    def calculate_blank_percentage(self, player: pd.Series) -> float:
        ...
        # Require recent points data
        if not recent_points:
            if "form" not in player.index:
                raise ValueError(
    ...
    def score_player(self, player: pd.Series) -> float:
        ...
        Returns:
            Player score
        """
        if "position" not in player.index:
            raise ValueError(f"Missing position for player {player.get('player_name', 'unknown')}")
        ...
        if position == "GK":
            ...
            raise ValueError(f"Invalid position: {position}. Must be GK, DEF, MID, or FWD")

        # Calculate weighted score
        score = 0
        for feature, weight in weights.items():
            if feature not in player.index:
                # Skip missing features rather than using 0
                continue
        ...
        if "chance_of_playing_next_round" not in player.index:
            raise ValueError(
                f"Missing chance_of_playing_next_round for player {player.get('player_name', 'unknown')}"
            )
    ...
    def score_all_players(self, players_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "has_real_odds" in players_df.columns:
            ...
            players_df = players_df[players_df["has_real_odds"]].copy()
        ...
        # Add overall rank
        players_df["overall_rank"] = players_df["model_score"].rank(ascending=False, method="dense")
...
def test_rule_based_scorer():
    ...
    # Test scorer with sample weights
