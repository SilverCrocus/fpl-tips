...
class Backtester:
    ...
    def _select_optimal_team(self, players_df: pd.DataFrame, budget: float) -> list[int]:
        ...
        for position, reqs in self.POSITIONS.items():
            if position_counts[position] < reqs["min"]:
                ...
                pos_players = candidates[
                    (candidates["position"] == position) & (~candidates["player_id"].isin(selected))
    ...
    def _calculate_team_cost(self, player_ids: list[int], players_df: pd.DataFrame) -> float:
    ...
    def _simulate_transfers(
        self, current_team: FPLTeam, current_gw_data: pd.DataFrame, free_transfers: int = 1
    ) -> tuple[FPLTeam, int]:
    ...
    def _calculate_gameweek_points(self, team: FPLTeam, gw_data: pd.DataFrame) -> int:
    ...
    def run_backtest(self, season: str, start_gw: int = 1, end_gw: int = 38) -> BacktestResult:
        ...
        final_team_value = (
            self._calculate_team_cost(self.current_team.players, final_gw_data)
    ...
    def compare_strategies(self, strategies: dict[str, any], season: str) -> pd.DataFrame:
