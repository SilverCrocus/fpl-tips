...
class FeatureEngineer:
    ...
    def engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if all(
            col in df.columns
            for col in ["fixture_difficulty", "fixture_diff_next2", "fixture_diff_next5"]
        ):
            # Lower is better for fixtures, so invert
            df["fixture_score"] = (
                (6 - df["fixture_difficulty"]) * 0.5
        ...
        if "position" in df.columns:
            ...
            if "saves" in df.columns and "minutes" in df.columns:
                df.loc[df["position"] == "GK", "saves_per_90"] = (df["saves"] / df["minutes"]) * 90
        ...
        if "prob_goal" in df.columns and "prob_assist" in df.columns:
            ...
            for pos, weight in position_weights.items():
                ...
                df.loc[mask, "position_adjusted_prob"] = (
                    df.loc[mask, "expected_involvement"] * weight
                )
...
class EnsemblePredictor:
    ...
    def predict(self, players_df: pd.DataFrame, horizon: str = "next_gw") -> np.ndarray:
        ...
        if not self.is_trained:
            raise RuntimeError(
                "Cannot make predictions: Models must be trained first. Run train() with historical data."
            )
    ...
    # Removed heuristic prediction method - no fallbacks allowed
...
class MILPOptimizer:
    ...
    def _add_constraints(
        ...
        players_df: pd.DataFrame,
        budget: float,
        current_team: Optional[list[int]],
        free_transfers: int,
    ):
        ...
        self.problem += (
            lpSum([prices[i] * self.variables["select"][i] for i in players]) <= budget,
            "Budget",
        )

        # 2. Squad size: exactly 15 players
        self.problem += lpSum([self.variables["select"][i] for i in players]) == 15, "Squad_Size"
        ...
        for position, (min_count, max_count) in [
            ...
            self.problem += (
                lpSum([self.variables["select"][i] for i in position_players]) == max_count,
        ...
        for team in teams:
            ...
            self.problem += (
                lpSum([self.variables["select"][i] for i in team_players]) <= 3,
                f"Team_{team}_Limit",
            )

        # 5. Captain constraints
        self.problem += lpSum([self.variables["captain"][i] for i in players]) == 1, "One_Captain"

        self.problem += (
            lpSum([self.variables["vice_captain"][i] for i in players]) == 1,
            "One_Vice_Captain",
        )
        ...
        for i in players:
            self.problem += (
                self.variables["captain"][i] <= self.variables["select"][i],
                f"Captain_Selected_{i}",
            )
            self.problem += (
                self.variables["vice_captain"][i] <= self.variables["select"][i],
                f"Vice_Selected_{i}",
            )
        ...
        for i in players:
            self.problem += (
                self.variables["captain"][i] + self.variables["vice_captain"][i] <= 1,
                f"Different_Captains_{i}",
            )

        # 6. Bench constraints: exactly 4 bench players
        self.problem += lpSum([self.variables["bench"][i] for i in players]) == 4, "Bench_Size"

        # Bench players must be selected
        for i in players:
            self.problem += (
                self.variables["bench"][i] <= self.variables["select"][i],
                f"Bench_Selected_{i}",
            )
        ...
        if current_team:
            ...
            # Number of transfers would be: lpSum([self.variables["transfer_out"][i] for i in current_team])
    ...
    def get_solution(self, players_df: pd.DataFrame) -> OptimizationResult:
        ...
        expected_points = sum(
            p["expected_points"] * (2 if p["is_captain"] else 1) for p in starting_xi
        )
...
class FPLOptimizer:
    ...
    def optimize_team(
        ...
        horizon_map = {"short_term": "next_gw", "balanced": "next_3_gws", "long_term": "next_5_gws"}
        if strategy not in horizon_map:
            raise ValueError(
                f"Invalid strategy '{strategy}'. Must be one of: {list(horizon_map.keys())}"
            )
        ...
        if not self.milp_optimizer.solve(time_limit=30):
            raise RuntimeError(
                "MILP optimization failed to find solution within time limit. Check constraints or increase time limit."
            )
    ...
    def _filter_candidates(
        self, players_df: pd.DataFrame, budget: float, top_n: int = 200
    ) -> pd.DataFrame:
        ...
        for position in ["GK", "DEF", "MID", "FWD"]:
            ...
            position_limits = {"GK": 10, "DEF": 40, "MID": 40, "FWD": 25}
            if position not in position_limits:
                raise ValueError(f"Unknown position: {position}")
            n_keep = position_limits[position]
    ...
    # Removed greedy selection method - no fallbacks allowed
    ...
    def optimize_transfers(
        ...
        for _, current_player in current_team_df.iterrows():
            ...
            if "expected_points" not in current_player:
                raise ValueError(
                    f"Missing expected_points for player {current_player.get('player_name', 'unknown')}"
                )
            current_points = current_player["expected_points"]
...
if __name__ == "__main__":
    ...
    if not players_df.empty:
        ...
        for p in [p for p in result.players if not p.get("is_bench", False)]:
            captain_mark = (
                " (C)" if p.get("is_captain") else " (VC)" if p.get("is_vice_captain") else ""
            )
            print(
                f"{p['position']:3} {p['player_name']:20} {p['team_name']:15} £{p['price']:.1f}m{captain_mark}"
            )
