...
class StrategicChipAdvisor:
    ...
    def detect_gameweek_type(self, team_data: pd.DataFrame) -> dict:
        ...
        if "is_dgw" in team_data.columns:
            ...
            dgw_players = team_data[team_data["is_dgw"]]["player_name"].tolist()
    ...
    def evaluate_triple_captain_strategic(
        self, team_data: pd.DataFrame, captain_analysis: dict
    ) -> dict:
        """
        ...
        if gw_info["is_dgw"]:
            ...
            if captain_name in gw_info.get("dgw_players", []):
                ...
                if is_premium_captain:
                    reasons.append("⭐ Premium captain in DGW - OPTIMAL timing!")
                    strategic_score += 5
    ...
    def evaluate_bench_boost_strategic(self, team_data: pd.DataFrame) -> dict:
        ...
        if self.scorer:
            ...
            team_scored = scored_data[
                scored_data["player_id"].isin(team_data["player_id"].tolist())
            ]
        else:
    ...
    def evaluate_wildcard_strategic(
        self, team_issues: list[dict], upcoming_dgw_bgw: bool = False
    ) -> dict:
        """
    ...
    def get_comprehensive_chip_strategy(
        self,
        team_data: pd.DataFrame,
        team_issues: list[dict],
        captain_analysis: dict,
        available_chips: dict,
    ) -> dict:
        """
    ...
    def _get_strategic_summary(self, chip_advice: dict) -> str:
        ...
        immediate_use = [chip for chip, advice in chip_advice.items() if advice.get("use")]
