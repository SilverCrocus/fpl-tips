...
class StrategicTransferEvaluator:
    ...
    def convert_model_score_to_expected_points(self, model_score: float, position: str) -> float:
        ...
        conversion_factors = {
            'GKP': 0.04,  # GKs typically score 2-6 points
        ...
        base_points = {
            'GKP': 2.0,  # Playing points + average saves
        ...
        max_points = {'GKP': 12, 'DEF': 15, 'MID': 20, 'FWD': 18}
        expected_points = min(expected_points, max_points.get(position, 15))
        
        return expected_points
    
    def calculate_point_gain(self, 
                           player_out: pd.Series, 
                           player_in: pd.Series,
                           horizon_weeks: int = None) -> float:
        """
    ...
    def evaluate_strategic_modifiers(self,
                                    player_out: pd.Series,
                                    player_in: pd.Series) -> Dict:
        """
    ...
    def evaluate_transfer_hit(self,
                             player_out: pd.Series,
                             player_in: pd.Series,
                             is_free_transfer: bool = False) -> Dict:
        """
        ...
        if modifiers['has_fixture_swing']:
            reasons.append(f"📈 Favorable fixture swing (diff: {modifiers['fixture_swing_value']:.1f})")
    ...
    def evaluate_multiple_hits(self,
                              transfers: List[Tuple[pd.Series, pd.Series]],
        ...
        if total_gain < total_cost:
            ...
            overall_advice = f'⚠️ Marginal benefit - consider fewer hits'
        else:
