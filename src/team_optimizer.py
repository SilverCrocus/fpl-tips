...
# Import the data merger and scorer
...
class TeamOptimizer:
    ...
    def get_player_scores(self, gameweek: Optional[int] = None) -> pd.DataFrame:
        ...
        if 'has_real_odds' in scored.columns:
            scored = scored[scored['has_real_odds'] == True]
    ...
    def optimize_team_milp(self, df: pd.DataFrame, strategy: str = 'balanced') -> Dict:
        ...
        for idx, row in df.iterrows():
            player_vars[row['player_id']] = pulp.LpVariable(
                f"player_{row['player_id']}", 
                cat='Binary'
            )
        
        # Objective: Maximize total team score
        prob += pulp.lpSum([
            player_vars[row['player_id']] * row['model_score']
            for idx, row in df.iterrows()
        ]), "Total_Team_Score"
        ...
        prob += pulp.lpSum([
            ...
            for idx, row in df.iterrows()
        ]) == self.constraints.squad_size, "Squad_Size"
        
        # Constraint 3: Position requirements
        for position, count in self.constraints.positions.items():
            # Handle both GK and GKP notation
            if position == 'GKP':
                pos_players = df[(df['position'] == 'GKP') | (df['position'] == 'GK')]
            else:
        ...
        for team in df['team'].unique():
            ...
            ]) <= self.constraints.max_per_team, f"Team_{team}_Limit"
        ...
        # Format team by position - handle both GK and GKP
        gk_players = selected_df[(selected_df['position'] == 'GKP') | (selected_df['position'] == 'GK')]
        
        team_by_position = {
            'GKP': gk_players.to_dict('records'),
    ...
    def _greedy_team_selection(self, df: pd.DataFrame) -> Dict:
        ...
        for idx, player in df.iterrows():
            # Map GK to GKP for consistency
            pos = 'GKP' if player['position'] == 'GK' else player['position']
    ...
    def recommend_team(self, strategy: str = 'balanced', gameweek: Optional[int] = None) -> Dict:
        ...
        try:
            ...
            print(f"Top scorer: {df.nlargest(1, 'model_score')[['player_name', 'model_score', 'price']].to_dict('records')[0]}")
