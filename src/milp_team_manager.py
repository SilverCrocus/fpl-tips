...
class MILPTransferOptimizer:
    ...
    def optimize_transfers(self, 
                          ...
                          max_transfers: int = 3,
                          horizon_weeks: int = 5) -> MILPTransferResult:
        """
        ...
        if 'has_real_odds' in all_players.columns:
            all_players = all_players[all_players['has_real_odds'] == True]
        ...
        max_sell_price = current_team.nlargest(1, 'price')['price'].iloc[0] if not current_team.empty else 0
        ...
        num_transfers = pulp.lpSum([
            1 - keep_vars[pid] for pid in current_team_ids
        ])
        ...
        prob += pulp.lpSum([
            keep_vars[pid] for pid in current_team_ids
        ]) + pulp.lpSum([
            buy_vars[pid] for pid in available_players['player_id']
        ]) == 15, "Squad_Size"
        ...
        for position, required_count in positions.items():
            # Handle both GK and GKP
            if position == 'GKP':
                current_pos = current_team[
                    (current_team['position'] == 'GKP') | 
                    (current_team['position'] == 'GK')
                ]
                available_pos = available_players[
                    (available_players['position'] == 'GKP') | 
                    (available_players['position'] == 'GK')
                ]
            else:
            ...
            count = pulp.lpSum([
                keep_vars[player['player_id']] 
                for _, player in current_pos.iterrows()
            ]) + pulp.lpSum([
                buy_vars[player['player_id']]
                for _, player in available_pos.iterrows()
            ])
        ...
        for team in all_teams:
            ...
            team_count = pulp.lpSum([
                keep_vars[player['player_id']]
                for _, player in current_team_players.iterrows()
            ]) + pulp.lpSum([
                buy_vars[player['player_id']]
                for _, player in available_team_players.iterrows()
            ])
        ...
        kept_value = current_team[
            current_team['player_id'].isin([
                pid for pid in current_team_ids 
        ...
        for out_player, in_player, net_cost in paired_transfers:
            ...
            if running_budget >= net_cost:
                ...
                import logging
                logging.warning(f"Transfer {out_player['name']} → {in_player['name']} "
...
class MILPCaptainSelector:
    ...
    def select_captain_and_vice(self, 
                               team_ids: List[int],
                               consider_effective_ownership: bool = True) -> Dict:
        """
        ...
        team_data = team_data[~team_data['position'].isin(['GK', 'GKP'])]
        ...
        for _, player in team_data.iterrows():
            captain_vars[player['player_id']] = pulp.LpVariable(
                f"captain_{player['player_id']}", 
                cat='Binary'
            )
            vice_vars[player['player_id']] = pulp.LpVariable(
                f"vice_{player['player_id']}", 
                cat='Binary'
            )
        ...
        total_score = pulp.lpSum([
            ...
            vice_vars[player['player_id']] * calculate_captain_score(player) * 0.3  # Vice worth 30% 
            for _, player in team_data.iterrows()
        ])
        ...
        prob += pulp.lpSum([
            captain_vars[pid] for pid in team_data['player_id']
        ]) == 1, "One_Captain"
        
        # 2. Exactly one vice-captain
        prob += pulp.lpSum([
            vice_vars[pid] for pid in team_data['player_id']
        ]) == 1, "One_Vice_Captain"
...
class MILPChipAdvisor:
    ...
    def evaluate_bench_boost(self, team_ids: List[int]) -> Dict:
        ...
        gks = team_data[team_data['position'].isin(['GK', 'GKP'])].nsmallest(1, 'model_score')
        ...
        if avg_bench_score > 5:  # Strong bench expected points
            reasons.append(f"Strong bench scoring potential ({bench_score:.1f} total points expected)")
            confidence += 3
    ...
    def evaluate_triple_captain(self, team_ids: List[int]) -> Dict:
        ...
        non_gk = team_data[~team_data['position'].isin(['GK', 'GKP'])]
        if non_gk.empty:
        ...
        if 'fixture_difficulty' in best_player and best_player['fixture_difficulty'] <= 2:
            reasons.append(f"Very favorable fixture (difficulty: {best_player['fixture_difficulty']})")
            confidence += 3
